...
for(j in 1:length(wealth.names)){
  ...
  # grafico
  png(filename= paste("D:/PABLO/Documentos (2TB)/WORK IN PROGRESS D/20 nodos y 30 rondas/Wealth/plot", wealth.names[j], " bars.png", sep = " ", collapse = NULL))
  
  par(mar=rep(5,4))
  
  errbar(x30, acum, acum+errorsd, acum-errorsd, col="blue", pch=15, xlim=c(0,30), ylim=c(0,3000), lwd=0.5, xlab="round", ylab="cummulated wealth", errbar.col="blue")
  par(new=TRUE)
  errbar(x30, acumC, acumC+errorsdC, acumC-errorsdC, col="darkorange", pch=17, xlim=c(0,30), ylim=c(0,3000), axes="FALSE", xlab="", ylab="", errbar.col="darkorange")
  par(new=TRUE)
  errbar(x30, acumR, acumR+errorsdR, acumR-errorsdR, col="olivedrab4", pch=18, xlim=c(0,30), ylim=c(0,3000), axes="FALSE", xlab="", ylab="", errbar.col="olivedrab4")
  par(new=TRUE)
  errbar(x30, RRtreat, RRtreat+errorRR, RRtreat-errorRR, col="black", pch=16, xlim=c(0,30), ylim=c(0,3000), axes="FALSE", xlab="", ylab="", errbar.col="black")
